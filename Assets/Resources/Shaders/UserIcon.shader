        ...
        pass
        {
            ...
            float inFlash(float angle,float2 uv,float xLength,int interval,int beginTime, float offX, float loopTime )
            {
                ...
                if(currentTimePassed >beginTime)
                {
                    ...
                    xPointLeftBound = xBottomLeftBound;
                    //此点的右边界 = 底部右边界 - 投影至x的长度
                    xPointRightBound = xBottomRightBound;
                    ...
                    //如果该点在区域内
                    if(uv.x > xPointLeftBound && uv.x < xPointRightBound && uv.y >0.19 && uv.y < 0.2)
                    {
                        brightness =0.5;
                        //得到发光区域的中心点
            ...
            float4 frag (v2f i) : COLOR
            {
                ...
                //传进i.uv等参数，得到亮度值
                float tmpBrightness;
                tmpBrightness =inFlash(75,i.uv,0.25f,5.0f,2.0f,0.15,2.5f);
